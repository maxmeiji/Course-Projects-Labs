# -*- coding: utf-8 -*-
"""
Created on Sun Dec 31 01:02:53 2023
...
def tsne(X=np.array([]), no_dims=2, initial_dims=50, perplexity=30, method = 0, labels = np.array([])):
    """
    ...
    for iter in range(max_iter):
        ...
        if iter == 100:
            P = P / 4.
    images = [Image.open(f"./method_{method}_{perplexity}/{i}.png") for i in range(max_iter//50)]        
    images[0].save(f"./method_{method}_{perplexity}/result.gif", save_all=True, append_images=images[1:], duration=100, loop=0)
