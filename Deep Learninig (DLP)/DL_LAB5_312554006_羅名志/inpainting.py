...
from faster_pytorch_fid.fid_score_iter import *
class MaskGIT:
    def __init__(self, args, MaskGit_CONFIGS):
        self.model = VQGANTransformer(MaskGit_CONFIGS["model_param"]).to(device=args.device)
        self.model.load_state_dict(torch.load(args.load_transformer_ckpt_path))
        self.model.eval()
...
#mask_b: iteration decoding initial mask, where mask_b is true means mask
    def inpainting(self,image,mask_b,i, epoch): #MakGIT inference
        ...
        self.sweet_spot = epoch
        self.model.eval()
        with torch.no_grad():
            
            __, z_indices = self.model.encode_to_z(image) #z_indices: masked tokens (b,16*16)
            ...
            plt.figure()
            plt.plot(range(self.sweet_spot), ratio_list)
            plt.xlabel('Epoch')
...
#TODO3 step1-2: modify the path, MVTM parameters
    parser.add_argument('--load-transformer-ckpt-path', type=str, default='./transformer_checkpoints/10_golden.pt', help='load ckpt')
    ...
    #MVTM parameter
    parser.add_argument('--sweet-spot', type=int, default=9, help='sweet spot: the best step in total iteration')
    parser.add_argument('--total-iter', type=int, default=13, help='total step for mask scheduling')
    ...
    for epoch in epochs_to_record: 
        print(epoch)       
        for image, mask in tqdm(zip(t.mi_ori, t.mask_ori), total=len(t.mi_ori), desc="Inpainting Images"):
            image=image.to(device=args.device)
            mask=mask.to(device=args.device)
            mask_b=t.get_mask_latent(mask)       
            maskgit.inpainting(image,mask_b,i, epoch)
            i+=1
        
        fid_score_iter = fid_score()
        fid_list.append(fid_score_iter)

    plt.figure()
    plt.plot(epochs_to_record, fid_list)
    plt.xlabel('Epoch')
    plt.ylabel('FID Score')
    plt.title('FID Score Plot')
