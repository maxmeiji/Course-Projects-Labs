...
class OxfordPetDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.transform is not None:
            ...
            image  = torch.from_numpy(np.moveaxis(sample['image'],-1,0)).float()
            # print(image.shape)
            ...
            image_afternorm = transform(image_pil).detach().numpy()
            # print(image_afternorm.shape)
            sample['image'] = np.moveaxis(image_afternorm, 0, -1).astype(np.uint8)
            # print(sample['image'].shape)
            return sample
        return sample
...
if __name__ == '__main__':
    root = os.getcwd()
    print(root)
    dir = os.path.join(root, os.path.join('dataset','os.oxford-iiit-pet/'))
    load_dataset(dir, 'test')
"""
